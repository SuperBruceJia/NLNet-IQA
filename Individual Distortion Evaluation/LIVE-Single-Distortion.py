...
def run(args):
    ...
    # LIVE
    LIVEinfo = mat73.loadmat('benchmark/LIVEfullinfo.mat')
    label_info = LIVEinfo["subjective_scores"]
    images_info = LIVEinfo["im_names"]

    distortion_types = ['jpeg', 'jp2k', 'wn', 'gblur', 'fastfading']
    for individual_type in distortion_types:
        srcc_all = []
        plcc_all = []
        for model_id in range(10):
            # Load model
            args.model_file = './save_model/LIVE-32-4-' + str(model_id + 1) + '.pth'
            ...
            pred_scores = []
            gt_scores = []
            file_dir = '/media/shuyuej/Projects/Dataset/live/' + individual_type + '/'
            for img_id in os.listdir(file_dir):
                if img_id != 'Thumbs.db' and img_id != 'info.txt':
            ...
            print(str(args.model_file) + ': LIVE Database: ', str(individual_type), ' - SRCC', srcc, ' - PLCC', plcc)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_file", default='save_model/LIVE-1.pth', type=str, help="model file (default: models/CNNIQA-LIVE)")
    ...
    parser.add_argument('--lr_decay_epoch', default=5, type=int, help='Learning rate decay after lr_decay_epoch (default: 10)')
    parser.add_argument('--num_workers', default=8, type=int, help='Number of workers to load training data (default: 2 X num_cores = 8)')

    # Choose Database
    parser.add_argument('--num_dis_type', default=5, type=int, help='Number of distortion types (default: TID2013: 24)')
    parser.add_argument('--database_path', default='/home/shuyuej/Desktop/Dataset/', type=str, help='Database Path (default: ./dataset/)')
